...
function danmakuMarkup(media) {
  const lines = (media.danmaku || []).slice(0, 30).map((item, index) => {
    const color = /^#[0-9a-f]{6}$/i.test(item.color || '') ? item.color : '#ffffff';
    return `<span class="danmaku-line" style="--danmaku-index:${index};--danmaku-color:${color}">${esc(item.content)}</span>`;
  }).join('');
  const visual = media.mediaType === 'video' ? `<video src="${esc(media.url)}" controls preload="metadata"></video>` : `<img src="${esc(media.url)}" alt="${esc(media.originalName)}" loading="lazy">`;
  return `<div class="danmaku-stage">${visual}<div class="danmaku-layer" aria-label="弹幕">${lines}</div></div>`;
}
...
function renderGuide() {
  app.innerHTML = `<section class="card hero guide-hero"><div class="hero-kicker">快速上手</div><h1>使用说明</h1><p>用几句话完成登记、上传、弹幕互动和素材管理。</p></section><section class="card guide-content"><div class="guide-step"><span>01</span><div><h2>AI 投稿</h2><p>进入“提交照片 / 视频”，输入“我是五连1班兰天笑，上传这张照片，名字叫我的中国心”，再选择图片或视频，AI会自动识别身份和展示名称。</p></div></div><div class="guide-step"><span>02</span><div><h2>查看相册</h2><p>进入公开相册，点击对应连队栏即可展开。图片和视频上会显示滚动弹幕。</p></div></div><div class="guide-step"><span>03</span><div><h2>发送弹幕</h2><p>在素材下方填写昵称和内容，点击“发弹幕”。弹幕会在图片或视频上方滚动展示。</p></div></div><div class="guide-step"><span>04</span><div><h2>管理自己的素材</h2><p>在AI助手中说“帮我删除我的中国心”，确认目标后即可删除。系统只允许删除当前用户自己的素材。</p></div></div><a class="guide-back" href="/">返回首页</a></section>`;
}
...
async function renderAllGallery() {
  ...
  const teamSections = data.activities.flatMap((activity) => activity.teams.map((team) => {
    const content = team.media.length
      ? `<div class="grid">${team.media.map((m) => `<article class="media-card" data-id="${m.id}">${danmakuMarkup(m)}<div class="media-body"><div class="media-name">${esc(m.originalName)} · ${fmtSize(m.sizeBytes)}</div><div class="actions"><button class="like secondary">点赞 <span>${m.likes}</span></button><button class="report secondary">举报</button></div><div class="comments">${(m.comments || []).map((c) => `<div class="comment"><strong>${esc(c.nickname)}</strong>${esc(c.content)}</div>`).join('')}</div><form class="comment-form"><input name="nickname" maxlength="30" placeholder="昵称" required><textarea name="content" maxlength="500" placeholder="写一句评论" required></textarea><button>评论</button></form><form class="danmaku-form"><input name="nickname" maxlength="30" placeholder="昵称" required><input name="content" maxlength="80" placeholder="发一条弹幕" required><button>发弹幕</button></form></div></article>`).join('')}</div>`
  ...
  collapseInteractionUI(); bindSimpleGalleryInteractions(); bindDanmakuInteractions();
}
...
function bindDanmakuInteractions() {
  app.querySelectorAll('.danmaku-form').forEach((form) => form.onsubmit = async (event) => {
    event.preventDefault(); const card = form.closest('.media-card'); const fields = new FormData(form); const button = form.querySelector('button');
    button.disabled = true; try { await api(`/media/${card.dataset.id}/danmaku`, { method:'POST', body:JSON.stringify({ nickname:fields.get('nickname'), content:fields.get('content') }) }); form.reset(); await renderAllGallery(); } catch (error) { alert(error.message); } finally { button.disabled = false; }
  });
}
...
async function renderGallery() {
  ...
  grid.innerHTML=data.media.map(m=>`<article class="media-card" data-id="${m.id}">${danmakuMarkup(m)}<div class="media-body"><div class="media-name" title="${esc(m.originalName)}">${esc(m.originalName)} · ${fmtSize(m.sizeBytes)}</div><div class="actions"><button class="like secondary">点赞 <span>${m.likes}</span></button><button class="report secondary">举报</button></div><div class="comments">${(m.comments||[]).map(c=>`<div class="comment"><strong>${esc(c.nickname)}</strong>${esc(c.content)}</div>`).join('')}</div><form class="comment-form"><input name="nickname" maxlength="30" placeholder="昵称" required><textarea name="content" maxlength="500" placeholder="写一句祝福" required></textarea><button>留言</button></form><form class="danmaku-form"><input name="nickname" maxlength="30" placeholder="昵称" required><input name="content" maxlength="80" placeholder="发一条弹幕" required><button>发弹幕</button></form></div></article>`).join(''); collapseInteractionUI(); bindDanmakuInteractions();
  const currentDeviceKey = deviceKey();
